...
def ParseAttributesToDict(attributes):
    attribute_dict = {}
...
def ParseTransdecoderGff3(gff3file):
    ...
    for line in gffopen:
        if line[0] != '#' and 'ID' in line:
            ...
            if line_dict['type'] in ['gene', 'mRNA']:
                line_dict['attributes'] = '%s"' % line_dict['attributes'].split(',')[0]
                attribute_dict=ParseAttributesToDict(line_dict['attributes'])
                if line_dict['type'] == 'gene':
                    attribute_dict['ID'] = attribute_dict['ID'].split('^')[0]
                    new_attributes = []
                    ...
                    attribute_dict['Parent'] = attribute_dict['Parent'].split('^')[0] 
                ...
                raise ValueError("%s unknown feature type for transdecoder gff3 file" % line_dict['type'])
